...
data Buffer u v w x y z q i j where
  ...
  B2 :: !(q i j) -> !(q j k) -> Buffer F F T F F F q i k
  B3 :: !(q i j) -> !(q j k) -> !(q k l) -> Buffer F F F T F F q i l
  B4 :: !(q i j) -> !(q j k) -> !(q k l) -> !(q l m) -> Buffer F F F F T F q i m
  B5 :: !(q i j) -> !(q j k) -> !(q k l) -> !(q l m) -> !(q m n) -> Buffer F F F F F T q i n

data Pair q i k where
  P :: !(q i j) -> !(q j k) -> Pair q i k

data HPair q r i k where
  H :: !(q i j) -> !(r j k) -> HPair q r i k

pushB :: q i j -> Buffer u v w x y F q j k -> Buffer F u v w x y q i k
pushB a B0 = B1 a
pushB a (B1 b) = B2 a b
...
pushB2 :: Pair q i j -> Buffer u v w x F F q j k -> Buffer F F u v w x q i k
...
injectB :: Buffer u v w x y F q i j -> q j k -> Buffer F u v w x y q i k
injectB B0 a = B1 a
...
injectB2 :: Buffer u v w x F F q i j -> Pair q j k -> Buffer F F u v w x q i k
...
data Fringe c q i j k l where
  RX :: !(Buffer s F F F F t q i j) -> !(Buffer u v w x y z q k l) -> Fringe Red q i j k l
  XR :: !(Buffer F u v w x F q i j) -> !(Buffer y F F F F z q k l) -> Fringe Red q i j k l
  YX :: !(Buffer F u F F v F q i j) -> !(Buffer F w x y z F q k l) -> Fringe Yellow q i j k l
  GY :: !(Buffer F F w x F F q i j) -> !(Buffer F y F F z F q k l) -> Fringe Yellow q i j k l
  GG :: !(Buffer F F w x F F q i j) -> !(Buffer F F y z F F q k l) -> Fringe Green q i j k l
...
push :: q i j -> Deque q j k -> Deque q i k
